...
class CreateUnionpayLogsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('unionpay_logs', function (Blueprint $table) {
            ...
            $table->string('msg_type', 2)->default('00')->comment('报文类型');
            ...
            $table->string('msg_time', 20)->nullable()->comment('报文日期');
            $table->string('msg_ver', 10)->nullable()->comment('报文版本号');
            $table->string('mkt_code', 100)->nullable()->comment('券码')->index();
            $table->string('msg_sys_sn', 40)->nullable()->comment('平台流水号');
            $table->string('req_serial_no', 50)->nullable()->comment('流水号');
            $table->string('orig_req_serial_no', 50)->nullable()->comment('原流水号');
